    ...
    */
    ...
    CalvinLazyLoad.prototype = {
        ...
        _setOptions: function (options) {
            ...
            return sl.extend(this.options, options || {});
        },
        //初始化容器设置
        _initContainer: function (container) {
            var doc = document, isWindow = sl.InstanceOf.BodyOrHtmlOrWindow(container);
            ...
            $(container).bind("scroll", sl.throttle(oThis.delay, function () {
                oThis._load.call(oThis);
            }, true));
            isWindow && $(container).bind("resize", sl.throttle(oThis.delay, function () {
                //是否已经改变了 宽度
        ...
        _initStatic: function (mode, direction) {
            ...
            this._setElems = function () {
                // var timer1 = "sfsaf";
                // console.time(timer1);
                this._elems = sl.map(this._elems, getRect).sort(sortFunction);
        ...
        dispose: function (load) {
            ...
            if (this._elems || this._binder) {
                //加载全部元素
                if (load && this._elems) {
                    sl.each(this._elemts, function (i, elem) {
