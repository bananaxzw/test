        ...
        var messageHelper = {
            ...
            */
            createMessage: function (target) {
                ...
                $(target).CalvinPanel({ noheader: !opts.isShowHeader, closable: opts.showCloseBtn, onClose: function () {
                    maskHelper.hideMasker(target);
                    opts.closed = true;
                }
                });

                return $(target).data("panel").panel;
            },
            /**
            * @description 包裹对话框和设置对话框的样式
            */
            wrapAndSetMessageStyle: function ($panel, target) {
                ...
                var $message;
                if (full) {
                    $message = $('<div class="CalvinUIBlock ' + opts.blockMsgClass + ' blockPage" style="z-index:' + z_index + 2 + ';display:none;position:fixed"></div>');
                }
                else {
                    $message = $('<div class="CalvinUIBlock ' + opts.blockMsgClass + ' blockElement" style="z-index:' + z_index + 2 + ';display:none;position:absolute"></div>');
                }
                ...
                if (CalvinBase.BrowserHelper.isIE6() && full) {
                    ...
                    $message.css(opts.css);
                    //$message[0].style.setExpression("top", '20+"px"');
