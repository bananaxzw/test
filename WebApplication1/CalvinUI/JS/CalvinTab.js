...
(function () {
    ...
    */
    ...
    $.fn.CalvinTabs = function (options, params) {
        var opts;
        ...
        var tabsHeaderHelper =
            {
                ...
                */
                createOneTab: function (target, options, index) {
                    ...
                    //绑定事件
                    tab.bind("click.tabs", { target: target, tab: tab }, eventHelper.selectTab);
                    return tab;
                },
                /**
                ...
                */
				...
				.appendTo($('>div.tabs-panels', target));
                    ...
                    $.data(target, "tabs").tabs.push(tab);
                    if (options.selected) {
                        tab.trigger("click");
                    }
                ...
                */
                getSelectedTab: function (container) {
                    var tabs = $.data(container[0], "tabs").tabs;
                    for (var i = 0, m = tabs.length; i < m; i++) {
                        var tabHeader = tabs[i][0];
                        var cacheOpt = $.data(tabHeader, "tabs.tab");
        ...
        var tabsStyleHelper = {
            ...
            */
            ...
            setPanelSize: function (target) {
                ...
                var header = $.data(target, "tabs").header;
                if (opts == undefined) {
                    opts = $.data(target, "tabs").options;
                }
        ...
        var eventHelper = {
            ...
            */
            selectTab: function (event) {
                var $this = event.data.tab;
                var target = event.data.target;
                var data = $.data(target, "tabs");
                var tabs = $('ul.tabs', header);
                var header = data.header;
                var TabOpt = $.data($this[0], "tabs.tab");
            ...
            */
            DefalutSelectTab: function (target, title) {
                if (title) {
                    var elem = $('>div.tabs-header li:has(a span:contains("' + title + '"))', target)[0];
                    if (elem) {
                        $(elem).trigger('click');
                    }
                } else {
        ...
        var dataCacheHelper = {
            /**
            * @description 1、移除  $.data(this, 'tabs', { options: opts, tabs: wrapTabs(this) }); 的tabs中的一个元素
            ...
            */
            ResetTabsIndex: function (container) {
                var tabs = $.data(container, 'tabs').tabs;
                for (var i = 0, m = tabs.length; i < m; i++) {
                    $.data(tabs[i][0], "tabs.tab").index = i;
                }
            },
            ...
            */
            setAllTabsDataUnSelected: function (target) {
                var opts = $.data(target, 'tabs');
                var tabs = opts.tabs;
                $.each(tabs, function (i, n) {
                    $.data(n[0], "tabs.tab").isSelect = false;
                });
        ...
        if (typeof options === "string") {
            ...
            switch (options.toUpperCase()) {
                case "ADD":
                    tabsHeaderHelper.addOneTab(this[0], params);
                    return;
                case "GETSELECTED":
                    return tabsHeaderHelper.getSelectedTab(this);
                case "SELECT":
                    return tabsHeaderHelper.selectTabByIndex(this[0], params);
        ...
        return this.each(function () {
            ...
            if (state) {
                ...
                var htmlAttr = {
                    ...
                    height: (parseInt(this.style.height) || undefined),
                ...
                opts = $.extend(true, defaults, htmlAttr, options);
                $this.data("tabs", { options: opts, container: null, header: null, panels: [], tabs: [] });

            }
            tabsPanelsHelper.wrapTabPanels(this);
            tabsHeaderHelper.createTabHeaders(this);
            tabsStyleHelper.setSize(this);
            eventHelper.DefalutSelectTab(this);
        });
    }
})();