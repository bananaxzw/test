...
(function () {
    ...
    var CalvinLazyLoad = function (elems, options) {
        //初始化程序
    ...
    CalvinLazyLoad.prototype = {
		...
		isWindow = container == window || container == doc
           ...
           // this.load = $$F.bind(this._load, this); //定义好load跟_load绑定
            ...
            this.delayLoad = function () { oThis._delay(oThis._load); }; //延时加载程序
        ...
        _loadStatic: function (direction, beforeRange, afterRange, force) {
            //获取方向
            direction = this[direction](force); //判断滚动的方向
            if (!direction) return;
            //根据方向历遍图片对象
            //水平和垂直_index好像一直保持在0
