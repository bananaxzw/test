...
easyTemplate.separate = function (s) {
    ...
    var sRet = s.replace(/(<(\/?)#(.*?(?:\(.*?\))*)>)|(')|([\r\n\t])|(\$\{([^\}]*?)\})/g, function (a, b, c, d, e, f, g, h) {
        //b  <#list data.list as list>   <#if (list_index % 2)>   </#if>
        // c   /
        // d #list data.list as list      (list_index % 2)   if
        // e '
        //f 换行
        // g ${list_index + 1}
        // h   list_index + 1
        if (b) {
            //"<#list data.list as list>"=> "{|}+list data.list as list{|}"
            return '{|}' + (c ? '-' : '+') + d + '{|}';
        }
        if (e) {
            return '\\\'';
    ...
    while (aList.length) {
        ...
        switch (aTmp[0]) {
            ...
            case '+else':
                ...
                break;
            case '+list':
                aStm.push('if(' + aTmp[1] + '.constructor === Array){with({i:0,l:' + aTmp[1] + '.length,'
                 + aTmp[3] + '_index:0,' + aTmp[3] + ':null}){for(i=l;i--;){' + aTmp[3] + '_index=(l-i-1);' + aTmp[3] + '=' + aTmp[1] + '[' + aTmp[3] + '_index];');
                break;
            case '-list':
                aStm.push('}}}'); break;
            default: break;
...
//    with ({ i: 0, l: data.list.length, list_index: 0, list: null }) {
//        for (i = l; i--; ) {
//            list_index = (l - i - 1); list = data.list[list_index]
//        }
//    }
//};